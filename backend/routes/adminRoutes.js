...
const getUserFilter = ({ search, role, status }) => {
  const filters = [];
...
const getPagination = (query) => {
  ...
  return {
    page: Number.isNaN(requestedPage) ? 1 : Math.max(requestedPage, 1),
...
const getActivityQuery = async ({ page, limit }) => {
  ...
  try {
    const [
      ...
      Like.find()
        ...
        .select("title artist artwork user createdAt")
        .sort({ createdAt: -1 })
        .limit(6)
        .lean(),
      Like.aggregate([
...
router.get("/users", protect, adminOnly, async (req, res) => {
  try {
    ...
    const filter = getUserFilter({ search, role, status });
...
router.get("/activity", protect, adminOnly, async (req, res) => {
  try {
    ...
    return res.json(await getActivityQuery({ page, limit }));
  } catch (error) {
...
router.post("/users", protect, adminOnly, async (req, res) => {
  try {
    ...
    const status = ["active", "suspended"].includes(req.body.status)
      ? req.body.status
      : "active";
    ...
    if (password.length < 6) {
      return res.status(400).json({
        message: "The temporary password must be at least 6 characters.",
      });
    }
...
router.patch("/users/:userId", protect, adminOnly, async (req, res) => {
  try {
    ...
    if ("name" in req.body) {
      ...
      if (!updates.name) return res.status(400).json({ message: "Name is required." });
    }
    if ("email" in req.body) {
      updates.email = String(req.body.email || "").trim().toLowerCase();
      if (!updates.email) return res.status(400).json({ message: "Email is required." });
    }
    ...
    return res.json({ user: normalizeUser(user.toObject()) });
  } catch (error) {
...
router.delete("/users/:userId", protect, adminOnly, async (req, res) => {
  try {
    ...
    return res.json({ message: "User and their saved-song records were deleted." });
  } catch (error) {
...
const updateUserField = async ({ req, res, field, allowedValues, action }) => {
  try {
    ...
    const previousValue =
      existingUser[field] || (field === "status" ? "active" : "user");
    ...
    return res.json({
      ...
      user: normalizeUser(updatedUser.toObject()),
    });
  } catch (error) {
    console.error("Admin user update error:", error);
...
router.patch("/users/:userId/role", protect, adminOnly, (req, res) =>
  updateUserField({
    req,
    res,
    field: "role",
...
router.patch("/users/:userId/status", protect, adminOnly, (req, res) =>
  updateUserField({
    req,
    res,
    field: "status",
